	...
	public void run() {
		...
		while (true) {
			...
			while (true) {
				...
				isConnected = ConnectUtil.isConnect();
				if (isConnected) {
					break;
				}
				
				logger.error("请注意！！ 网络连接已断开！！请重新连接");
				try {
					Thread.sleep(1000);
			...
			try {
				...
				if (null != entity) {
					...
					while (m.find()) {
						...
						redisDataSource.setValueinSetCollection(RedisCacheType.USER_URL, user);
						logger.info("用户 :" + user + "已经被存入了redis数据库");
					}
